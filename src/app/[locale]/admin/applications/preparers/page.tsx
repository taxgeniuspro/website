...
export default function PreparerApplicationsPage() {
  ...
  const openActionDialog = (application: PreparerApplication, action: 'approve' | 'reject') => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold">Tax Preparer Applications</h1>
        <p className="text-muted-foreground mt-2">Review and manage tax preparer applications</p>
      </div>
                ...
                filteredApplications.map((application) => (
                  <TableRow key={application.id}>
                    ...
                    <TableCell>
                      {application.experienceLevel ? (
                        <Badge variant="outline">
                          {experienceLevelLabels[application.experienceLevel] || application.experienceLevel}
                        </Badge>
                      ) : (
                        <span className="text-muted-foreground text-sm">Not specified</span>
                      )}
                    </TableCell>
                    <TableCell>{application.languages}</TableCell>
                    ...
                    <TableCell>
                      ...
                      {application.status === 'APPROVED' && <Badge variant="default">Approved</Badge>}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {actionDialog.action === 'approve'
                ? 'This will create a tax preparer account and grant them access to the platform.'
          ...
          {actionDialog.application && (
            <div className="space-y-4">
              <div className="rounded-lg border p-4 space-y-2">
                ...
                {actionDialog.application.taxSoftware.length > 0 && (
                  <div className="flex justify-between">
                    <span className="text-sm font-medium">Software:</span>
                    <span className="text-sm">{actionDialog.application.taxSoftware.join(', ')}</span>
                  </div>
                )}
              </div>
