...
/**
 * Ollama Client Wrapper for TaxGeniusPro SEO Brain
 *
 * Handles all communication with Ollama API for local LLM text generation
 * Cost: $0 (runs locally)
 */
...
export class OllamaClient {
  ...
  async generateJSON<T = any>(options: OllamaGenerateOptions): Promise<T> {
    ...
    try {
      ...
      logger.error('[Ollama] JSON parse error:', error);
      logger.error('[Ollama] Raw response:', response);
  ...
  async getModels(): Promise<string[]> {
    try {
      ...
      logger.error('[Ollama] Failed to fetch models:', error);
      return [];
    }
  }
}
