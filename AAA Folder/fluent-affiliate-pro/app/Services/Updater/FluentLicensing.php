...
namespace FluentAffiliatePro\App\Services\Updater;
...
class FluentLicensing
{
    ...
    public function getLicenseMessage()
    {
        ...
        $activateUrl = $this->config['activate_url'] ?? '';

        if ($status == 'expired') {
            return [
                'message'         => 'Your FluentAffiliate license has been' . ' <b>' . 'expired at' . ' ' . gmdate('d M Y', strtotime($licenseDetails['expires'])) . '</b>, Please ' .
                    '<a href="' . $activateUrl . '"><b>Click Here to Renew Your License</b></a>',
        ...
        if ($status != 'valid') {
            return [
                'message'         => sprintf(
                    'The FluentAffiliate license needs to be activated. %1$sActivate Now%2$s',
                    '<a href="' . $activateUrl . '">',
                    '</a>'
                ),
                'license_details' => $licenseDetails
